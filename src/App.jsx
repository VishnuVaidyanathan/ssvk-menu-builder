...
const ADDONS = [
  { id: "addon_flowers", name: "பூ மற்றும் மாலை (Flowers & Garlands)", enabled: false },
  { id: "addon_thamboola", name: "தாம்பூல பை (Thamboola Bags)", enabled: false },
  { id: "addon_purohit", name: "புரோஹித் (Purohit)", enabled: false },
  { id: "addon_nadaswaram", name: "நாதஸ்வரம் (Nadaswaram)", enabled: false },
  { id: "addon_banana_leaves", name: "வாழை இலை (Banana Leaves)", enabled: false },
  { id: "addon_coconuts", name: "தேங்காய் (Coconuts)", enabled: false },
  { id: "addon_water", name: "தண்ணீர் பாட்டில் (Water Bottles)", enabled: false },
  { id: "addon_icecream_stall", name: "ஐஸ்கிரீம் ஸ்டால் (Ice Cream Stall)", enabled: false },
  { id: "addon_beeda_stall", name: "பீடா ஸ்டால் (Beeda Stall)", enabled: false },
];
...
function generatePDFContent(template, customerName, startDate, endDate, addons, bakshanam) {
  let content = "";
  content += "ஸ்ரீ சிவா விஷ்ணு கேட்டரிங் காண்ட்ராக்ட்ஸ்\n";
  content += "Cell No.: 9842408584 | 8248066570\n";
  content += "═══════════════════════════════════════\n\n";
...
export default function CateringMenuBuilder() {
  ...
  const [addons, setAddons] = useState(() => deepClone(ADDONS));
  ...
  const handleToggleAddon = (addonId) => {
    setAddons((prev) =>
      prev.map((a) => (a.id === addonId ? { ...a, enabled: !a.enabled } : a))
    );
  };
  ...
  const previewContent = generatePDFContent(template, customerName, startDate, endDate, addons, bakshanam);
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: "600px", margin: "0 auto", padding: "16px" }}>
        ...
        {/* Meals for Active Day or Bakshanam */}
        {activeDay === "bakshanam" ? (
          ...
          {addons.map((addon) => (
            ...
            >
              <div style={{
                ...
                minWidth: "22px",
                borderRadius: "4px",
                border: addon.enabled ? "2px solid #b8860b" : "2px solid #ccc",
                background: addon.enabled ? "#b8860b" : "white",
                color: "white",
                fontSize: "14px",
                display: "flex",
                alignItems: "center",
              ...
              <span style={{
                fontSize: "14px",
                color: addon.enabled ? "#333" : "#888",
                fontFamily: "'Noto Sans Tamil', sans-serif",
                fontWeight: addon.enabled ? "600" : "400",
              }}>
                {addon.name}
              </span>
            </div>
        ...
        <div style={{ display: "flex", gap: "10px", marginBottom: "16px" }}>
          ...
          <button
            onClick={() => {
              ...
              a.href = url;
              a.download = `menu_${customerName || "customer"}_${startDate || "draft"}.txt`;
              a.click();
        ...
        <div style={{ textAlign: "center", paddingBottom: "40px" }}>
          <button
            onClick={() => {
              setTemplate(deepClone(MEAL_TEMPLATES["3-day-brahmin"]));
              setCustomerName("");
              setStartDate("");
              setEndDate("");
              setAddons(deepClone(ADDONS));
